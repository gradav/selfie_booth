            ...
            canvas.toBlob(async (blob) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // Start 10-second countdown before returning to kiosk (longer wait for mobile review)
                        let photoCount = 10;
...
<body>
    ...
    <script>
        ...
        function startPhotoCheck() {
            photoCheckInterval = setInterval(checkForPhoto, 2000); // Check every 2 seconds
        }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Redirect to photo session page
                    setTimeout(() => {
                        if (result.redirect) {
                            window.location.href = result.redirect;
                        }
...
def kiosk():
    ...
    # Check if there's a verified user ready for photo (within 3 minutes) OR a retake request
    cursor.execute('''
...
def photo_session():
    ...
    conn = sqlite3.connect('selfie_booth.db')
    cursor = conn.cursor()
    cursor.execute('SELECT verified, first_name FROM sessions WHERE session_id = ?', (session['session_id'],))
...
def admin_config():
    ...
    if recent_sessions:
        ...
        for session in recent_sessions:
            session_id, name, phone, verified, code, photo_ready, created_at = session
            try:
                if 'T' in created_at:
                    created_time = datetime.fromisoformat(created_at)
                else:
